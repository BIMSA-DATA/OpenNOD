...
main_excel_file = os.path.join(pDir, 'AGI/All_Tweets_list_AGI.xlsx')
agi_minute_start = '2022-10-01 00:00:00'
agi_minute_end = '2023-10-05 23:59:59'
...
def count_objTweets_By_minhour(result_sheet, minhour='minute'):
    ...
    # 自动获取时间列表
    minute_list, hour_list = get_minute_hour_list(agi_minute_start, agi_minute_end)

    # 加载已有的工作簿
    wb = load_workbook(main_excel_file)
    # 检查是否存在名为 objnews_statis_hour 的工作表
    ...
    for timestr in time_list:
        ...
        for index, twettime in enumerate(twttime_list):
            if twettime.startswith(timestr):
                ...
                if (';;impossible;;' in stance_str):
                    stance_value1 = -1
                    stance_value2 = -2
                elif (';;unlikely;;' in stance_str):
                    stance_value1 = -1
                    stance_value2 = -1
                elif (';;possible;;' in stance_str):
                    stance_value1 = 1
                    stance_value2 = 1
                elif (';;certain;;' in stance_str):
                    stance_value1 = 1
                    stance_value2 = 2
                else:
                    stance_value1 = 0
...
def count_tweets_stance_by_minhour(sheetname, tweetstance_count_result_filepath, minhour='hour'):
...
def Linear_fit_stance_change(stance_counts_user_minute_filename, tweets_stance_views_minute_interval_filename,
    ...
    resultfile = open(projectDir+'AGI//model_fitting_results//fit_result_'+str(gij_type)+'_'+str(gij_value)+'_'+str(en_type)+'_'+str(en_value)+'.txt', 'w', encoding='utf-8')
...
def newsheat_stance_by_user_interval(stance_counts_filename, objnews_statis_sheetname,
                                   tweets_stance_views_minhour_interval_filename, newsheat_result_filename, weight=1):
    from openpyxl import Workbook
    wb = Workbook()
    ws = wb.active
...
def filter_non_na_tweets():
    ...
    # 获取“stance_new”不包含 ";;na;;" 的所有数据
    filtered_df = df[~df['stance_new'].str.contains(";;na;;", na=False)]
...
def filter_subjboth_tweets():
    ...
    filtered_df = df[
        (~df['stance_new'].str.contains(';;na;;', na=False)) &
...
if __name__ == '__main__':
    ...
    # step 1: 计算tweets与新闻（0.AGI_news）的相关度，并选出相关度高的objtweets。
    # 在excel文件里进行标记，列名为 'mark_obj'。
    # 在mark_objtweets_by_news-sim_AGI.py中实现（需要GPU服务器）。
    
    # step 2: 按小时/分钟统计时间内的各项数据，
    # 存入 main_excel_file 的 名为'objnews_statis_hour'的sheet
    # count_objTweets_By_minhour('objnews_statis_hour', 'hour')
    ...
    # step 4: 按小时统计tweets的 stance 分布
    # count_tweets_stance_by_minhour('Tweets_wStance_SortByTime', pDir+'/AGI/TweetStance_Counts_byHours.json', 'hour')

    # step 5: 按user/小时统计tweets的 stance 分布
    # count_tweets_stance_by_user_minhour('Tweets_wStance_SortByUser', pDir+'/AGI/TweetStance_Counts_byUserHour.json', 'hour')

    # step 6: 计算user两条推文之间的 news heat（新闻热度）
    # newsheat_by_user_2tweets_interval(pDir+'/AGI/TweetStance_Counts_byUserHour.json', 'objnews_statis_hour', pDir+'/AGI/Newsheat_byUser2TweetsInterval_hour.json')
    ...
    # step 8. 按 小时 对subj推文进行统计。计算每一小时内的
    # 五个类别的推文的推文数/浏览量/评论量/点赞数/转发量。
    # count_tweets_by_stance_views_minhour('Tweets_subjboth', pDir+'/AGI/SubjBothTweets_byStanceViewsHour.json', 'hour')

    # step 9. 对用户两次 tweets 之间的 subj推文 进行分析。
    # 计算 五个类别的推文的推文数/浏览量/评论量/点赞数/转发量 的 
    # 最大值/中位数/75分位数/平均值。
    # subjtweets_by_stance_views_interval(pDir+'/AGI/TweetStance_Counts_byUserHour.json',
    #                                     pDir+'/AGI/SubjBothTweets_byStanceViewsHour.json',
    #                                     pDir+'/AGI/SubjBothTweets_byStanceViewsInterval_hour.json')

    # step 10. 对用户两次发言之前的新闻指数进行统计，
    # 计算两次发言之间的新闻/交叉新闻指数：推文数/浏览量/评论数的最大值/中位数。
    # newsheat_stance_by_user_interval(pDir + '/AGI/TweetStance_Counts_byUserHour.json',
    #                                  'objnews_statis_hour',
    #                                  pDir + '/AGI/SubjBothTweets_byStanceViewsInterval_hour.json',
    #                                  pDir + '/AGI/Newsheat_Stance_byUserInterval_2_hour.xlsx', 2)

    # step 11. 对subjtweets进行分析，按分钟统计 每种stance 的 推文数，浏览量，评论数
    count_subjtweets_by_stance_minhour('Tweets_subjboth', pDir+'/AGI/subjtweets_statis_hour.xlsx', 'hour')