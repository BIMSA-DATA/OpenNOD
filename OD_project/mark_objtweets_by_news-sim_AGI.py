...
def mark_tweets(df, news_dict):
    ...
    for news_date, news_list in news_dict.items():
        ...
        end_date = news_date + timedelta(days=100)
        ...
        # 💡 Encode tweets once
        tweet_embeddings = model.encode(tweet_texts, batch_size=64, convert_to_tensor=True, device=device)
        ...
        for news in news_list:
            ...
            for i, score in enumerate(scores):
                if score > 0.45 and df.at[tweet_indices[i], 'mark_obj1'] == 0:
                    df.at[tweet_indices[i], 'mark_obj1'] = 1

            print(f"  > matched {sum(scores > 0.45)} tweets")
...
if __name__ == "__main__":
    ...
    excel_file_path = project_path + 'AGI/All_Tweets_list_AGI.xlsx'
    # Please replace it with the actual news folder path.
    news_folder_path = project_path + 'AGI/0.AGI_news'
